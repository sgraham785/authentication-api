...
const User = Bookshelf.Model.extend({
  ...
  login: Promise.method(function (email, password) {
    ...
    return new this({ email: email.toLowerCase().trim() }).fetch({ require: true }).tap(user => {
      ...
      return hashword.compareAsync(password, user.get('password')).then((matched, err) => {
  ...
  register: Promise.method(data => {
    ...
    return Joi.validate(data, schema).then((validated, err) => {
